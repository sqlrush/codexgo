...
func (s *LocalThreadStore) listThreadsFromStateDB(ctx context.Context, params ListThreadsParams, pageSize int) (ThreadPage, bool, error) {
	if s.stateDB == nil {
		return ThreadPage{}, false, nil
	}

	anchor, err := anchorFromCursor(params.Cursor)
	if err != nil {
		return ThreadPage{}, false, err
	}
	...
	if err != nil {
		return ThreadPage{}, false, internalError(err, "failed to list threads")
	}
	if dbPage == nil || len(dbPage.Items) == 0 {
		return ThreadPage{}, false, nil
	}

	items := make([]StoredThread, 0, len(dbPage.Items))
	for i := range dbPage.Items {
		items = append(items, s.storedThreadFromSQLiteMetadata(&dbPage.Items[i]))
	}

	return ThreadPage{Items: items, NextCursor: cursorFromAnchor(dbPage.NextAnchor)}, true, nil
}
...
// memory.
func (s *LocalThreadStore) listThreadsFromScan(ctx context.Context, params ListThreadsParams, pageSize int) (ThreadPage, error) {
	...
	if err != nil {
		return ThreadPage{}, internalError(err, "failed to scan sessions for thread listing")
	}

	threads := make([]StoredThread, 0, len(paths))
...
func threadMatchesListFilters(thread StoredThread, params ListThreadsParams) bool {
...
func sortStoredThreads(threads []StoredThread, key ThreadSortKey, direction SortDirection) {
...
func paginateStoredThreads(threads []StoredThread, params ListThreadsParams, pageSize int) ThreadPage {
	start := 0
	...
	page := make([]StoredThread, end-start)
	copy(page, threads[start:end])
	return ThreadPage{Items: page, NextCursor: nextCursor}
}

// cursorStartIndex returns the index of the first thread strictly after the
// cursor timestamp under the given ordering.
func cursorStartIndex(threads []StoredThread, cursorTS time.Time, key ThreadSortKey, direction SortDirection) int {
	for i := range threads {
...
func sortTimestamp(thread StoredThread, key ThreadSortKey) time.Time {
	if key == ThreadSortKeyUpdatedAt {
...
// stored thread.
func cursorFromStoredThread(thread StoredThread, key ThreadSortKey) string {
...
func toStateSortKey(key ThreadSortKey) state.SortKey {
	if key == ThreadSortKeyUpdatedAt {
...
func toStateSortDirection(direction SortDirection) state.SortDirection {
	if direction == SortDirectionAsc {
...
func anchorFromCursor(cursor *string) (*state.Anchor, error) {
	...
	if !ok {
		return nil, invalidRequestError("invalid cursor: %s", *cursor)
	}
