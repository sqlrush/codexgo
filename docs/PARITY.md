...
| `/responses` `tools` specs + order | `codex exec --json` vs codexgo (captured POST body) | ✅ **Pass (5/11 advertised)** — see `TestParityToolSpecs` + `TestParityToolOrder`. With the UnifiedExec bridge, codexgo resolves the shell tool family per turn via the ported `shell_type_for_model_and_features` (the `unified_exec` feature defaults on for PTY-capable hosts, **overriding** gpt-5.5's catalog `shell_type = "shell_command"` — this is why real codex advertises the PTY pair), so both binaries advertise `exec_command` + `write_stdin` backed by real PTY sessions, with `shell_command` registered **dispatch-only** (hidden), exactly like codex's spec_plan. All five advertised specs (`exec_command`, `write_stdin`, `update_plan`, `apply_patch` custom grammar, `view_image`) are **byte-identical**, and codexgo's advertised order is an ordered subsequence of codex's. Fixed a real drop-in divergence: the extra `shell_command` advertisement is gone. Remaining unadvertised: `get_goal`/`create_goal`/`update_goal`, `request_user_input`, `tool_search`, `web_search`. |
