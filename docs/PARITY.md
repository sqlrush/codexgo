...
| `/responses` `input` context | `codex exec --json` vs codexgo (captured POST body) | ✅ **Pass (modulo skills)** — see `TestParityInputContext`. codexgo seeds the codex initial-context messages into a new thread's history: the developer-role `<permissions instructions>` and the user-role `<environment_context>`. The item count, per-item roles, and the **permissions + environment_context content are byte-identical** to the real codex binary (same host ⇒ same cwd/date/timezone/sandbox). Fixed a real bug: `codex exec` defaults approval to `never` (not on-request). Remaining: codex bundles a second `<skills_instructions>` content part (~4.9 KB, `SKILL.md` scan) into the first message that codexgo does not yet render. |
| Full `/responses` request body | `codex exec --json` vs codexgo (captured POST body) | ⚠️ **Partial** — see `TestParityRequestBody`. The broadest request-side differential: it captures the full POST `/responses` body from both binaries for a plain turn and compares **every top-level field**. The key set matches, and these fields are now **byte-identical**: `model`, `tool_choice`, `store`, `stream`, `include`, `service_tier`, `text`, `reasoning`, `parallel_tool_calls`, **`instructions`**. It found+fixed **three more** real drop-in bugs: (3) `parallel_tool_calls` always sent `false` — now derived from `model_info.supports_parallel_tool_calls` (gpt-5.5 → `true`), matching codex (`compact_remote.rs`); (4) `reasoning` sent `"summary":"auto"` while codex omits it — now resolved as `config.model_reasoning_summary` ?? `model_info.default_reasoning_summary` (gpt-5.5 → `"none"`, so no summary), matching codex (`turn_context.rs`); (5) `instructions` sent gpt-5.5's `base_instructions` verbatim, which **bakes the "friendly" personality**, while codex renders the model's `instructions_template` with the **resolved personality** — `cli.or(config.personality).or(Pragmatic)` since the Personality feature is on by default (`config/mod.rs`). `buildResponsesClientConfig` now calls `GetModelInstructions(resolvePersonality(...))`, so the base prompt is byte-identical (gpt-5.5 → "pragmatic"). **Two large request-shape ports remain tracked gaps** (`documentedGapFields`): `input` (codex prepends an **environment-context** user message — `<permissions instructions>` + cwd/sandbox/approval/network; codexgo sends only the bare turn; 6,298 vs 83 bytes), and `tools` (codex advertises the full gpt-5.5 **tool registry** of 11 tools; codexgo now advertises the UnifiedExec PTY pair + 3 utility tools as an **ordered subsequence** with byte-identical specs — see the tool-spec row below; goals/`tool_search`/`web_search` remain). |
| `/responses` `tools` specs + order | `codex exec --json` vs codexgo (captured POST body) | ✅ **Pass (7/11 advertised)** — see `TestParityToolSpecs` + `TestParityToolOrder`. With the UnifiedExec bridge, codexgo resolves the shell tool family per turn via the ported `shell_type_for_model_and_features` (the `unified_exec` feature defaults on for PTY-capable hosts, **overriding** gpt-5.5's catalog `shell_type = "shell_command"` — this is why real codex advertises the PTY pair), so both binaries advertise `exec_command` + `write_stdin` backed by real PTY sessions, with `shell_command` registered **dispatch-only** (hidden), exactly like codex's spec_plan. `request_user_input` is advertised by default (codex's `experimental_request_user_input_enabled` is true when unconfigured; description embeds the Plan-mode availability from `request_user_input_available_modes`), and the hosted `web_search` spec is advertised with the cached-mode default (`external_web_access:false` + gpt-5.5's `text_and_image` content types). All seven advertised specs (`exec_command`, `write_stdin`, `update_plan`, `request_user_input`, `apply_patch` custom grammar, `view_image`, `web_search`) are **byte-identical**, and codexgo's advertised order is an ordered subsequence of codex's. Fixed a real drop-in divergence: the extra `shell_command` advertisement is gone. Remaining unadvertised: `get_goal`/`create_goal`/`update_goal`, `tool_search`. |
