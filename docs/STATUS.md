...
| 36–40 | TUI | 🟡 | bubbletea chat/overlays/slash/keymap/onboarding; behavioral port. **Pixel-fidelity wave 2 landed**: the idle first frame is now cell-identical end-to-end. codexgo runs INLINE (no alt-screen): finalized history cells are printed into native scrollback via `tea.Println` (`ScrollbackDrainer`/`Model.withScrollback`) while the live viewport renders only the top inset + composer block. The composer matches codex's bordered-less `Min(3)` block (blank pad / `› <text-or-placeholder>` / blank pad) + default status-line footer `model-with-reasoning · current-dir`, with the random 8-entry placeholder pool mirrored. At 80×24 and 120×40 (`CODEX_TUI_FRAME_STRICT=1`) every non-masked row is byte-identical; only 4 rows stay masked (version, header dir, random placeholder, footer cwd), down from 9. Tooltip disabled in the harness for both (network/time-volatile). Remaining (wave 3): SGR-attribute comparison, streaming/resize/clear frame tests, real CLI version |
...
Rough faithful-and-verified completeness: **~95%**.
