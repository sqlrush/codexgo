...
- `VERSION` 文件是版本唯一来源,经 ldflags 注入二进制
  (`codexgo --version` 可见);
- 未经验证的提交**不出本机**;
- **每个验证过的版本都有 git 标签**(v0.1.0 起全量补齐),标签即回退锚点。

### 4.1 回退机制(两层)

| 层级 | 操作 | 耗时 | 影响 |
|------|------|------|------|
| **二进制回退** | `./scripts/rollback-local.sh v0.2.2` | 秒级 | 只换部署的二进制(临时 worktree 构建),仓库分支/工作树不动,可边用旧版边修新版 |
| **源码回退** | `git revert <range>` 或从标签拉分支 | 分钟级 | 回滚代码线;严禁 force-push 改写 main 历史 |

`rollback-local.sh` 不带参数可列出全部可回退版本。

### 4.2 大规模功能集成的分支策略(opendb 阶段起)

```
main ──────●────────●──────────────●──────▶  稳定线:只接收验证通过的合并
           │v0.2.2  │v0.3.0        │v0.3.1      每版打标签
           │        ▲              ▲
           └─ feat/opendb-xxx ─────┘          功能分支:开发 + 本地部署验证
```

- **main 永远可用**:任何时刻从 main 构建都是可工作的版本;
- 大功能在 `feat/opendb-<功能名>` 分支开发,`deploy-local.sh` 在分支上
  构建验证;改坏了直接弃分支,main 无感;
- 验证通过 → 合入 main → 升 VERSION → 打标签 → 推送;
- opendb 集成从 **0.3.x** 版本线开始,每个功能波次升一个次版本;
- 出现已发布版本的问题:先 `rollback-local.sh` 恢复可用,再在分支上修复。
