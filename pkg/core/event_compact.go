...
func maybePreSamplingAutoCompact(ctx context.Context, sess *Session, tc *TurnContext) {
	// The 0.147 decision is the shared context-window status (scope-aware,
	// buffered limit, hard cap) instead of a bare total >= limit check.
	if !contextWindowTokenStatus(sess, tc).TokenLimitReached {
		return
	}
	if ctx.Err() != nil {
		return
